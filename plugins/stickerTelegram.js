...
let handler = async (m, { conn, args, usedPrefix, command }) => {
    if (!args[0]) throw `*This command to retrieve stickers from Telegram*\n\nexample:\n${usedPrefix + command} https://t.me/addstickers/menggokil`
    if (!args[0].match(/(https:\/\/t.me\/addstickers\/)/gi)) throw `*wrong link! This command to retrieve stickers from Telegram*\n\nexample:\n${usedPrefix + command} https://t.me/addstickers/menggokil`
    ...
    m.reply(`*Total:* ${json.result.length}\n*Estimated complete:* ${json.result.length * 2} seconds\n\n` + mes)
